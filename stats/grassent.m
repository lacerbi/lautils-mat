...
G = NaN(1,1+max(h));
G(h+1) = real(psi(h) + 0.5*(-1).^h .*(psi(0.5*(h+1)) - psi(0.5*h)));
G(1) = 0;

% Note: Added real() to prevent imaginary numbers from popping up in PSI
% (this didn't happen before, something messed up with PSI?)
