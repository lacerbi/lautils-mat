function [samples,fvals,exitflag,output] = slicesamplebnd2(logf,x0,N,widths,LB,UB,options,varargin)
%SLICESAMPLEBND Slice sampling method with bounds and rotations.
%
...
if ~isempty(options.Burnin) && options.Burnin == 0 && isempty(widths) && options.Adaptive
...
funccount = 0;
[y,fval,logprior] = feval(@logpdfbound, x0, options.MinTemperature, varargin{:});   % First evaluation at X0
xx = x0;
...
burn = floor(options.Burnin) + (floor(options.Burnin)-1)*(thin-1);
log_Px = y;
...
xx_sum = zeros(1,D);
xx_sqsum = zeros(1,D);
% Store burnin samples for adaptation
if burn > 0 && options.Adaptive
    samples = zeros(options.Burnin, D);
else
    samples = zeros(N, D);
end

ismpl = 1;
Mdiag = eye(D);     % Diagonal covariance matrix
Mchol = [];         % Full covariance

% Main loop
for ii = 1:(effN+burn)
    
    temperature = min(1,(2*(1-options.MinTemperature)*(ii-1)/(burn-1))+options.MinTemperature);
    ...
    for dd = 1:D
        ...
        rr = rand;
        if isempty(Mchol) || rand() < 0.5
            wvec = Mdiag(dd, :).*widths;
        else
            wvec = Mchol(dd, :);
        end
        wsize = 1;
        ...
        if options.StepOut
            steps = 0;
            while (feval(@logpdfbound, x_l, temperature, varargin{:}) > log_uprime)
                x_l = x_l - wvec;
                wsize = wsize + 1;
                rr = rr + 1;
                steps = steps + 1;
            end
            while (feval(@logpdfbound, x_r, temperature, varargin{:}) > log_uprime)
                x_r = x_r + wvec;
                wsize = wsize + 1;
                rr = rr + 1;
                steps = steps + 1;
        ...
        while 1
            ...
            rr2 = rand()*wsize;
            xprime = x_l + rr2*wvec;
            [log_Px,fval,logprior] = feval(@logpdfbound, xprime, temperature, varargin{:});
    ...
    if ii <= burn && ii >= burn/2 && options.Adaptive
        xx_sum = xx_sum + xx;
        xx_sqsum = xx_sqsum + xx.^2;
                
        if ii == ceil(burn/2)       % Half-way update of covariance matrix
            Mchol = options.SigmaFactor*chol(cov(samples(1:ismpl-1,:)));
        elseif ii == burn           % End of burn-in, update WIDTHS
            burnstored = floor(burn/2);
            newwidths = options.SigmaFactor*sqrt(xx_sqsum/burnstored - (xx_sum/burnstored).^2);
            ...
            Mchol = options.SigmaFactor*chol(cov(samples(ceil(size(samples,1)/2):end,:)));
...
if options.Diagnostics && (nargout > 2 || trace > 0)
    [exitflag,R,Neff,tau] = diagnose(N,samples,trace,options);
    diagstr = [];
...
%--------------------------------------------------------------------------
function [y,fval,logprior] = logpdfbound(x,temperature,varargin)
%LOGPDFBOUND Evaluate log pdf with bounds and prior.
    ...
    if any(x < LB | x > UB)
        ...
        if any(isnan(fval))
            ...
            y = temperature*sum(fval) + logprior;        
        end
    end
end
...
function [exitflag,R,Neff,tau] = diagnose(N,samples,trace,options)
%DIAGNOSE Perform quick-and-dirty diagnosis of convergence
    ...
    try        
        ...
        warning('off','all');
        [R,Neff,~,~,~,tau] = psrf(samples(1:floor(Nsmp/2),:), samples(floor(Nsmp/2)+(1:floor(Nsmp/2)),:));
